...
export default class ModifierSelectUiHandler extends AwaitableUiHandler {
  ...
  setup() {
    ...
    this.lockRarityButtonText = addTextObject(this.scene, -4, -2, i18next.t('modifierSelectUiHandler:Lock_Rarities'),TextStyle.PARTY);
  ...
  setCursor(cursor: integer): boolean {
    ...
    if (this.rowCursor) {
      ...
      ui.showText(i18next.t('modifierSelectUiHandler:Spendtoreroll'));
    } else if (cursor === 1) {
      this.cursorObj.setPosition((this.scene.game.canvas.width / 6) - 50, -60);
      ui.showText(i18next.t('modifierSelectUiHandler:Transfer_a_held_item_to_another'));
    } else {
      this.cursorObj.setPosition(6, -60);
      ui.showText(i18next.t('modifierSelectUiHandler:Lock_item'));
    }
